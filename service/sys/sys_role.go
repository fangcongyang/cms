...
func GetUserRoleIdAndPermission(userId int64) (err error, role model.SysRoleIdPermissionVo) {
	var r model.SysRoleIdPermissionVo
	err = global.GVA_DB.Table("sys_role").Select("GROUP_CONCAT(id) AS RoleId, GROUP_CONCAT(permission) AS permission").Where("id IN (SELECT role_id FROM sys_user_role WHERE sys_user_id = ?)", userId).Find(&r).Error
	return err, r
}
