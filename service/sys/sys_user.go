...
func Register(u model.SysUser) (err error, userInter model.SysUser) {
	...
	if !errors.Is(global.GVA_DB.Where("username = ?", u.Account).First(&user).Error, gorm.ErrRecordNotFound) { // 判断用户名是否注册
...
func Login(u *model.SysUser) (err error, userInter *model.SysUser) {
	...
	isOk := utils.ComparePasswords(user.Password, []byte(u.Password))
	if isOk {
		_, role := GetUserRoleIdAndPermission(user.Id)
...
func ChangePassword(u *model.SysUser, newPassword string) (err error, userInter *model.SysUser) {
	var user model.SysUser
	u.Password = utils.MD5V([]byte(u.Password))
	err = global.GVA_DB.Where("username = ? AND password = ?", u.Account, u.Password).First(&user).Update("password", utils.MD5V([]byte(newPassword))).Error
	return err, u
}
