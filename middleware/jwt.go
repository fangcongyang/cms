...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if service.IsBlacklist(token) {
			response.Result(consts.TOKEN_INVALID, gin.H{"reload": true}, "您的帐户异地登陆或令牌失效", c)
			c.Abort()
			return
		}
		j := NewJWT()
		...
		if err != nil {
			if err == TokenExpired {
				response.Result(consts.TOKEN_EXPIRED, gin.H{"reload": true}, "授权已过期", c)
				c.Abort()
				return
			}
		...
		if claims.ExpiresAt-time.Now().Unix() < claims.BufferTime {
			...
			if global.GVA_CONFIG.System.UseMultipoint {
				...
				// 无论如何都要记录当前的活跃状态
				_ = service.SetRedisJWT(newToken, newClaims.Account)
			}
		}
		c.Set("claims", claims)
		c.Next()
